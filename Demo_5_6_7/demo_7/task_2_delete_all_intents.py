...
myResponse = requests.get(url + '/intents', auth=HTTPBasicAuth('onos', 'rocks'))
...
if myResponse.status_code == requests.codes.ok:
    ...
    for id in lists:
        print("Deleting intents", id['id'])
        myRespon = requests.delete(url + '/intents/' + id['appId'] + '/' + id['key'],
                                   auth=HTTPBasicAuth('onos', 'rocks'))
        print("Pass!:",myResponse)
